...
`endif
  ...
  reg         MDU_Started_r;
  wire        MDU_Stall, MDU_Start, Is_Mext_Div_E;
  wire [31:0] MDU_SrcA, MDU_SrcB;

  assign Is_Mext_Div_E = Is_Mext_E & MDU_Op_E[2];
  assign MDU_Stall     = Is_Mext_Div_E & ~MDU_Ready;
  assign MDU_Start     = Is_Mext_E & (~MDU_Op_E[2] | ~MDU_Started_r);

  always @(posedge Clk_In) begin
    if (Rst_In || !Is_Mext_Div_E)
      MDU_Started_r <= 1'b0;
    else if (MDU_Start)
      MDU_Started_r <= 1'b1;
    else if (MDU_Ready)
